...
public final class JDBCUtils {
	...
	public static <T> T select(Connection c, String sql, ResultSetHandler<T> resulSettHandler, Object... parameters)
			throws SQLException {

		try (PreparedStatement ps = c.prepareStatement(sql)) {
			populatePreparedStatement(ps, parameters);
			ResultSet rs = ps.executeQuery();
			return resulSettHandler.handle(rs);
		}
	}
	...
	public static void insertBatch(Connection c, String sql, List<Object[]> paramList ) throws SQLException {
		try (PreparedStatement ps = c.prepareStatement(sql)){
			for (Object[] pram : paramList) {
				populatePreparedStatement(ps, pram);
				ps.addBatch();
			}
			ps.executeBatch();
		}
		// TODO пакетная обработка запроса
