;$(function() {
	var init = function() {
		...
		$('.remove-product').click(removeProductFromCard);
		$('.post-request').click(function(){ postRequest($(this).attr('data-url'));
		});
	...
	var showAddProductPopup = function() {
		...
		$('#addProductPopup .product-image').attr('src',product.find('.img-thumbnail img').attr('src'));
	...
	var addProductToCart = function() {
		...
		$.ajax({
			...
			success : function(data) {
				$('#currentShopingCart .total-count').text(data.totalCount);
				$('#currentShopingCart .total-cost').text(data.totalCost);
				$('#currentShopingCart').removeClass('hidden');
	...
	var loadMoreProducts = function() {
		...
		var pageNumber = parseInt($('#prductlist').attr('data-page-number'));
		var url = '/ajax/html/more' + location.pathname + '?page='
				+ (pageNumber + 1) + '&' + location.search.substring(1);
		$.ajax({
					url : url,
					success : function(html) {
						$('#prductlist .row').append(html);
						pageNumber++;
						var pageCount = parseInt($('#prductlist').attr(
								'data-page-count'));
						if (pageNumber < pageCount) {
							$('#prductlist').attr('data-page-number',pageNumber);
	...
	var removeProductFromCard = function() {
		var btn = $(this);
	...
	var refreshTotalCost = function() {
		var total = 0;
		$('#shopingCart .item').each(function(index, value) {
			...
			success : function(data) {
				if(data.totalCount == 0){
					...
					if(remCount >= prevCount ){
						...
						convertLoaderToButton(btn, 'btn-danger', removeProductFromCard );
